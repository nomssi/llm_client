...
CLASS zcl_llm_encryption IMPLEMENTATION.
  METHOD zif_llm_encryption~decrypt.
    ...
    CALL FUNCTION 'SSF_KRN_DEVELOPE'
      ...
      EXCEPTIONS ssf_krn_error                = 1
                 ...
                 OTHERS                       = 11.
    IF sy-subrc <> 0.
      RAISE EXCEPTION NEW zcx_llm_validation( textid = zcx_llm_validation=>decryption_failed
                                              attr1  = CONV #( sy-subrc ) ).
    ENDIF.
  ...
  METHOD zif_llm_encryption~encrypt.
    ...
    input_data = VALUE #( FOR j = 0 THEN j + bin_line WHILE j < input_len
                          ( bindata = COND xstring(
    ...
    CALL FUNCTION 'SSF_KRN_ENVELOPE'
      ...
      EXCEPTIONS ssf_krn_error                = 1
                 ...
                 OTHERS                       = 11.
    IF sy-subrc <> 0.
      RAISE EXCEPTION NEW zcx_llm_validation( textid = zcx_llm_validation=>encryption_failed
                                              attr1  = CONV #( sy-subrc ) ).
    ENDIF.
  ...
  METHOD class_constructor.
    ...
    IF sy-subrc <> 0 OR profile IS INITIAL.
      RAISE EXCEPTION NEW zcx_llm_validation( textid = zcx_llm_validation=>encryption_setup
                                              attr1  = 'Filename'
