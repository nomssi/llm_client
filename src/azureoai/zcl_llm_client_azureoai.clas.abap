"! <p class="shorttext synchronized" lang="en">Azure OpenAI Client</p>
CLASS zcl_llm_client_azureoai DEFINITION
  ...
  PROTECTED SECTION.
    CONSTANTS:
      ...
      END OF status,
      client_name TYPE string VALUE `Azure_OpenAI` ##NO_TEXT,
      api_version type string value '2024-10-21'.
    ...
    TYPES:
      BEGIN OF function,
        name      TYPE string,
        "OpenAI returns the arguments as escaped JSON string.
      ...
      BEGIN OF openai_message,
        role       TYPE string,
        content    TYPE string,
        tool_calls TYPE STANDARD TABLE OF tool_call WITH EMPTY KEY,
      END OF openai_message,

      BEGIN OF openai_choice,
        finish_reason TYPE string,
        message       TYPE openai_message,
      END OF openai_choice,

      BEGIN OF openai_usage,
        prompt_tokens     TYPE i,
        completion_tokens TYPE i,
        total_tokens      TYPE i,
      END OF openai_usage,

      BEGIN OF openai_response,
        id      TYPE string,
        choices TYPE STANDARD TABLE OF openai_choice WITH EMPTY KEY,
        usage   TYPE openai_usage,
        model   TYPE string,
      END OF openai_response.
...
CLASS zcl_llm_client_azureoai IMPLEMENTATION.
  ...
  METHOD zif_llm_client~chat.
    TRY.
        msg = msg + 1.
        "Azure OpenAI services have the url .../openai/deployments/{deploymentName}/chat/completions?api-version={API-Version}
        "The deploymentName is mapped to the internal model name
        "API-Version is hard-coded
        client->set_url( |/{ client_config-provider_model }/chat/completions?api-version={ api_version }| ).
  ...
  METHOD handle_http_response.
    ...
    DATA response TYPE openai_response.
  ...
  METHOD zif_llm_client~new_request.
    ...
    request-structured_output = NEW zcl_llm_so_js_azureoai( ).
  ...
  METHOD zif_llm_client~get_client.
    response = NEW zcl_llm_client_azureoai( client_config = client_config provider_config = provider_config ).
  ENDMETHOD.

  METHOD set_auth.
    "OpenAI uses API Key in a Authorization: Bearer HeaderS
    ...
    IF provider_config-auth_type = 'A'.
      client->set_header( name = 'api-key' value = auth_value ) ##NO_TEXT.
    ENDIF.
  ENDMETHOD.
